...
def correct_dims(image, mask=None):
    corr_images = []

    if image is not None:
        if len(image.shape) == 2:
            image = np.expand_dims(image, axis=2)
        corr_images.append(image)

    if mask is not None:
        if len(mask.shape) == 2:
            mask = np.expand_dims(mask, axis=2)
        corr_images.append(mask)

    if len(corr_images) == 1:
        return corr_images[0], None
    else:
        return corr_images[0], corr_images[1]
